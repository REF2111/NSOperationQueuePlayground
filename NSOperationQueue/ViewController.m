...
typedef enum {
    ...
    DownloadFailed,
    NoDownloadedVendorListAvailable
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.purposeListNotAvailableReason = NoDownloadedVendorListAvailable;
...
- (NSString*)getStringForPurposeListNotAvailableReason:(PurposeListNotAvailableReason)purposeListNotAvailableReason
{
    switch (purposeListNotAvailableReason) {
        ...
        case DownloadFailed:
            ...
            break;
        case NoDownloadedVendorListAvailable:
            return @"No downloaded vendor list available";
            break;
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    if (self.operationQueue.operations.count == 0) {
        ...
        NSLog(@"Vendor list available with version: %@", self.vendorList[@"vendorListVersion"]);
        if (self.purposeList) {
            NSLog(@"Purpose list available with version: %@", self.purposeList[@"vendorListVersion"]);
        } else {
